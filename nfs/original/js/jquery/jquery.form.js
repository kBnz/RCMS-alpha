...
/*
	...
	j$(document).ready(function() {
		j$('#myForm').bind('submit', function(e) {
...
j$.fn.ajaxSubmit = function(options) {
	...
	var url = j$.trim(this.attr('action'));
	if (url) {
	...
	if (options.data) {
		...
		for (n in options.data) {
			if(options.data[n] instanceof Array) {
				...
				v = options.data[n];
				v = j$.isFunction(v) ? v() : v; // if value is fn, invoke it
	...
	if (!options.dataType && options.target) {
		...
		callbacks.push(function(data) {
			...
			j$(options.target)[fn](data).each(oldSuccess, arguments);
		});
	}
	...
	var fileInputs = j$('input:file', this).length > 0;
   ...
   if (options.iframe !== false && (fileInputs || options.iframe || multipart)) {
	...
	function fileUpload() {
		...
		if (j$(':input[name=submit],:input[id=submit]', form).length) {
		...
		var s = j$.extend(true, {}, j$.ajaxSettings, options);
		...
		var $io = j$('<iframe id="' + id + '" name="' + id + '" src="'+ s.iframeSrc +'" onload="window[\'_\'+this.id]()" />');
		var io = $io[0];
		...
		if (g) {
			j$.event.trigger("ajaxSend", [xhr, s]);
		}
			...
			try {
				if (s.extraData) {
					for (var n in s.extraData) {
						extraInputs.push(
							j$('<input type="hidden" name="'+n+'" value="'+s.extraData[n]+'" />')
								.appendTo(form)[0]);
					}
				}
		...
		function cb() {
			...
			try {
				...
				var isXml = s.dataType == 'xml' || doc.XMLDocument || j$.isXMLDoc(doc);
				log('isXml='+isXml);
				...
				data = j$.httpData(xhr, s.dataType);
			}
			catch(e){
				log('error caught:',e);
				ok = false;
				xhr.error = e;
				j$.handleError(s, xhr, 'error', e);
			}
			...
			if (ok) {
				...
				if (g) {
					j$.event.trigger("ajaxSuccess", [xhr, s]);
				}
			}
			if (g) {
				j$.event.trigger("ajaxComplete", [xhr, s]);
			}
			if (g && ! --j$.active) {
...
j$.fn.ajaxForm = function(options) {
	...
	if (this.length === 0) {
		...
		if (!j$.isReady && o.s) {
			...
			j$(function() {
				j$(o.s,o.c).ajaxForm(options);
			});
			return this;
		}
		// is your DOM ready?  http://docs.jquery.com/Tutorials:Introducing_j$(document).ready()
...
j$.fn.formToArray = function(semantic) {
	...
	for(i=0, max=els.length; i < max; i++) {
		...
		if (semantic && form.clk && el.type == "image") {
			...
			if(!el.disabled && form.clk == el) {
				a.push({name: n, value: j$(el).val()});
				a.push({name: n+'.x', value: form.clk_x}, {name: n+'.y', value: form.clk_y});
			}
			continue;
		}

		v = j$.fieldValue(el, true);
	...
	if (!semantic && form.clk) {
		...
		var $input = j$(form.clk), input = $input[0];
		n = input.name;
...
// set j$.fn.ajaxSubmit.debug to true to enable debug logging
function log() {
