...
var edit = {
	...
	initialize: function() {
		var self = this;
		var getLists = this.base2 + '/js/parts/getLists.php';
	...
	/** サイズ自動設定画面を開く */
	showSizer: function() {
		var url = this.base2 + '/direct.php?mt=parts&ct=display&sizer&' + this.po.serialize('GET');
		window.open(url, 'parts_sizer');
	},
	/** プレビュー更新＆コード生成 */
	refresh: function() {
		if ($F(this.content) && $F(this.template)) {
			...
			prv.src = this.base2 + '/direct.php?mt=parts&ct=display&' + this.po.serialize('GET');
			
			var code = 'var rcms_parts=' + this.po.serialize('JSON');
			code = '<script type="text/javascript">'+code+'</script>';
			code += '<script type="text/javascript" src="'+this.base+'/js/parts/rcms_parts.js"></script>';
