...
class CKFinder_Connector_CommandHandler_Thumbnail extends CKFinder_Connector_CommandHandler_CommandHandlerBase
{
    ...
    public function sendResponse()
    {
        ...
        //RCMS　カスタマイズ　使ってる？
        $tmp_server_path =  preg_replace('/[^\w\/\.:_\-]+/', '', $this->_currentFolder->getServerPath());

        if(substr($this->_currentFolder->getServerPath(),0,strlen(PHOTO_SAVE_DIR."/")) == PHOTO_SAVE_DIR."/"){
            $sourceFilePath = CKFinder_Connector_Utils_FileSystem::combinePaths(PHOTO_SAVE_DIR."/", $fileName);
            $_config =& CKFinder_Connector_Core_Factory::getInstance("Core_Config");
            $_thumbnailsConfig = $_config->getThumbnailsConfig();

            $thumbFilePath = CKFinder_Connector_Utils_FileSystem::combinePaths($_thumbnailsConfig->getDirectory(), filemtime($sourceFilePath)."_".$fileName);

        }elseif(substr($tmp_server_path,0,strlen(ROOT_DIR."/html/files/user_img/")) == ROOT_DIR."/html/files/user_img/"){
            $sourceFilePath = CKFinder_Connector_Utils_FileSystem::combinePaths($tmp_server_path, $fileName);
            $thumbFilePath = CKFinder_Connector_Utils_FileSystem::combinePaths(preg_replace('/[^\w\/\.:_\-]+/', '', $this->_currentFolder->getThumbsServerPath()), filemtime($sourceFilePath)."_".$fileName);

        }else{
            $sourceFilePath = CKFinder_Connector_Utils_FileSystem::combinePaths($this->_currentFolder->getServerPath(), $fileName);
            $thumbFilePath = CKFinder_Connector_Utils_FileSystem::combinePaths($this->_currentFolder->getThumbsServerPath(), filemtime($sourceFilePath)."_".$fileName);
        }
    ...
    public static function createThumb($sourceFile, $targetFile, $maxWidth, $maxHeight, $quality, $preserverAspectRatio, $bmpSupported = false)
    {
        ...
        make_dir( $path_parts['dirname'],true);
        return makeThumbnail($sourceFile, $targetFile, $maxWidth, $maxHeight);
